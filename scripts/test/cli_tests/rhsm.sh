...
if [ -z `which subscription-manager | grep -w no` ]; then
  if [ $PRINT_ALL -eq 1 ]; then
	  echo $RHSM_REGISTER
	  result=`$RHSM_REGISTER`
  else
	  printf "%-40s" "rhsm registration"
  ...
  echo "${txtred}subscription-manager not found in PATH\nskipping rhsm test(s)${txtrst}"
fi
