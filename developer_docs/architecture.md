...
- **Content View Environment**: The pairing of a content view with a lifecycle environment (i.e., a specific content view version's presence in a specific lifecycle environment). Abbreviate as `cvenv` (Ruby) / `cvEnv` (JavaScript) in code, never `cve` - "CVE" already means Common Vulnerabilities and Exposures in Katello's errata/security code (see the naming rule in `CLAUDE.md`, enforced for Ruby by the `Katello/CveAbbreviation` rubocop cop).
