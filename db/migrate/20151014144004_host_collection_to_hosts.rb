class HostCollectionToHosts < ActiveRecord::Migration
  ...
  class Katello::HostCollection < ActiveRecord::Base
  ...
  class Katello::HostCollectionHosts < ActiveRecord::Base
  ...
  def up
    ...
    Katello::SystemHostCollections.all.each do |system_host_collection|
      system = Katello::System.find(system_host_collection.system_id)
      host_collection = Katello::HostCollection.find(system_host_collection.host_collection_id)

      host_collection_host = Katello::HostCollectionHosts.new(:host_id => system.host_id, :host_collection_id => host_collection.id)
  ...
  def down
    ...
    Katello::HostCollectionHosts.all.each do |host_collection_host|
      system = Katello::System.where(:host_id => host_collection_host.host_id).first
      host_collection = Katello::HostCollection.find(host_collection_host.host_collection_id)

      system_host_collection = Katello::SystemHostCollections.new(:system_id => system.id, :host_collection_id => host_collection.id)
