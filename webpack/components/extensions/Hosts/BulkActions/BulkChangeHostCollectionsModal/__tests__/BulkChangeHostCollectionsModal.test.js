...
test('renders modal when open', async () => {
  ...
  await patientlyWaitFor(() => {
    ...
    expect(getByText('Add to host collection(s)')).toBeInTheDocument();
    expect(getByText('Remove from host collection(s)')).toBeInTheDocument();
  });
...
test('shows success toast notification on successful save', async () => {
  ...
  const saveScope = nockInstance
    ...
    .reply(200, { displayMessages: ['Host collections updated successfully'] });
