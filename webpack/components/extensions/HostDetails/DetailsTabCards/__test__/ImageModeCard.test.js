...
const providerProps = {
  cardExpandStates: { "Image mode details": true },
  dispatch: () => {},
  registerCard: () => {},
};
...
describe('image mode card', () => {
  test('shows image mode details when expanded', () => {
    ...
    const hostDetails = { ...baseHostDetails };
    const { getByText }
      = customRender(<ImageModeCard hostDetails={baseHostDetails} />, { providerProps });
  ...
  test('does not show details when not expanded', () => {
    const providerProps = {
      cardExpandStates: { "Image mode details": false },
      dispatch: () => {},
      registerCard: () => {},
    };
    const hostDetails = { ...baseHostDetails };
    const { getByText }
      = customRender(<ImageModeCard hostDetails={baseHostDetails} />, { providerProps });
    const element = getByText((_, element) => {
      return element.textContent ==='Running image'
    });
