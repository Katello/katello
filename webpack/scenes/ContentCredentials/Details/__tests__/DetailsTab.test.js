...
afterEach(async () => {
  // Ensure React components are properly unmounted
  cleanup();
...
beforeEach(() => {
  // Ensure clean nock state before each test
  nock.cleanAll();

  // Safety mocks with persist() to never get exhausted - using raw nock for this
  nock('http://localhost')
    .persist()
    .get(/\/katello\/api\/v2\/content_credentials\/\d+/)
    .query(true)
    .optionally()
    .reply(200, baseDetails);

  nock('http://localhost')
    .persist()
    .put(/\/katello\/api\/v2\/content_credentials\/\d+/)
    .optionally()
    .reply(200, baseDetails);

  nock('http://localhost')
    .persist()
    .post(/\/katello\/api\/v2\/content_credentials\/\d+\/content/)
    .optionally()
