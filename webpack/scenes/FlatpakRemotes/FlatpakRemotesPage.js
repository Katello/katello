...
const FlatpakRemotesPage = () => {
  ...
  const apiOptions = { key: 'FLATPAK_REMOTES' };
  const defaultParams = {
    ...response, page, perPage, organization_id: orgId(),
  };

  const apiResponse = useTableIndexAPIResponse({
    apiUrl: '/katello/api/v2/flatpak_remotes',
    apiOptions,
    defaultParams,
  });
  ...
  return (
    <TableIndexPage
      apiUrl="/katello/api/v2/flatpak_remotes"
      apiOptions={{ key: 'FLATPAK_REMOTES' }}
              ...
              {results.map((remote) => {
                ...
                return (
                  <Tr key={id} ouiaId={`flatpak-remote-row-${id}`}>
                    <Td>
                      <Link to={`${urlBuilder('flatpak_remotes', '')}${id}`}>
                        {truncate(name)}
                      </Link>
                    </Td>
                    <Td>
