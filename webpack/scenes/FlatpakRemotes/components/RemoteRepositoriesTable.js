...
const RemoteRepositoriesTable = ({ frId }) => {
  const columnHeaders = [__('Name'), __('ID'), __('Content'), __('Last mirrored'), __('Mirror')];

  const COLUMNS_TO_SORT_PARAMS = {
    [columnHeaders[0]]: 'name',
    [columnHeaders[1]]: 'id',
    [columnHeaders[2]]: 'content',
    [columnHeaders[3]]: 'last_mirrored',
  };
            ...
            <Thead>
              <Tr ouiaId="remoteReposTableHeaderRow">
                {columnHeaders.map(col => (
                  <Th key={col} {...(col === __('Mirror') ? {} : { sort: pfSortParams(col) })}>
                    {col}
                  </Th>
                ))}
              </Tr>
            ...
            <Tbody>
              {results.map(repo => (
                <Tr key={repo.id} ouiaId={`remote-repo-row-${repo.id}`}>
                  ...
                  <Td>{repo.id}</Td>
                  <Td>
                    {`${repo.tag_count} ${repo.tag_count === 1 ? __('Container tag') : __('Container tags')}`}<br />
                    {`${repo.manifest_count} ${repo.manifest_count === 1 ? __('Container manifest') : __('Container manifests')}`}
                  </Td>
                  <Td>
                    <LastSync
                      lastSyncWords={repo.last_mirrored?.last_sync_words}
                      lastSync={{
                        id: repo.last_mirrored?.sync_id,
