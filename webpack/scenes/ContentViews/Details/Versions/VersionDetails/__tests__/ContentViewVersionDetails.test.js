...
const withCVRoute = component => <Route path="/labs/content_views/:id/versions/:versionId">{component}</Route>;
...
const renderOptions = {
  ...
  routerParams: {
    initialEntries: [{ pathname: '/labs/content_views/3/versions/73' }],
    initialIndex: 1,
  },
};
...
test('Can show versions details - Components Tab', async (done) => {
  ...
  const { getByText, queryByText } = renderWithRedux(
    withCVRoute(<ContentViewVersionDetails />),
    renderOptions,
  );
