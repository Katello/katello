...
const withCVRoute = component => <Route path="/labs/content_views/:id/versions">{component}</Route>;

const renderOptions = {
  apiNamespace: `${CONTENT_VIEWS_KEY}_1`,
  routerParams: {
    initialEntries: [{ pathname: '/labs/content_views/5/versions' }],
    initialIndex: 1,
  },
};
...
test('Can call API and show versions on page load', async (done) => {
  ...
  const { getByText, queryByText } = renderWithRedux(
    withCVRoute(<ContentViewVersions />),
    renderOptions,
  );
