...
const CVDeletionReassignHostgroupsForm = () => {
  ...
  const {
    ...
    setSelectedCVNameForHostgroups, setSelectedCVForHostgroups, cvDetailsResponse,
    ...
    <>
      ...
      >
        <DescriptionList isHorizontal>
          <DescriptionListGroup>
            <DescriptionListTerm>{__('Affected host groups')}</DescriptionListTerm>
            <DescriptionListDescription>
              {hostgroups.length > 0 ? (
                <ul>
                  {hostgroups.map(hg => (
                    <li key={hg.id}>{hg.name}</li>
                  ))}
                </ul>
              ) : __('No host groups affected')}
            </DescriptionListDescription>
          </DescriptionListGroup>
        </DescriptionList>
      </ExpandableSection>
