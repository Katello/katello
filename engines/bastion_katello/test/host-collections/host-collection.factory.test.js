describe('Factory: HostCollection', function() {
    ...
    it('provides a way to list hosts', function() {
        var hosts = {results: [{id: 1}, {id: 2}]};
        $httpBackend.expectGET('/katello/api/host_collections/0/hosts').respond(hosts);
        HostCollection.hosts({id: 0}, function(response) {
            expect(response).toBeDefined();
            expect(response.length).toBe(hosts.length);
        });
    });
