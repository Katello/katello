/**
 * @ngdoc object
 * @name  Bastion.content-views.controller:DebFilterController
 *
 * @requires $scope
 * @requires $location
 * @requires translate
 ...
 *
 * @description
 *   Handles deb package filter rules for a content view.
 */
angular.module('Bastion.content-views').controller('DebFilterController',
    ['$scope', '$location', 'translate', 'Nutupane', 'CurrentOrganization', 'Filter', 'Rule', 'Deb', 'Notification', '$uibModal',
     function ($scope, $location, translate, Nutupane, CurrentOrganization, Filter, Rule, Deb, Notification, $uibModal) {
        var nutupane, params;
        ...
        $scope.getMatchingContent = function (rule) {
            $uibModal.open({
                templateUrl: 'content-views/details/filters/views/filter-rule-matching-deb-modal.html',
        ...
        $scope.fetchAutocompleteName = function (term) {
            ...
            promise = Deb.autocompleteName({'repoids[]': repositoryIds, term: term}).$promise;
        ...
        $scope.fetchAutocompleteArch = function (term) {
            ...
            promise = Deb.autocompleteArch({'repoids[]': repositoryIds, term: term}).$promise;
        ...
        $scope.filterRepositoriesByType = function () {
            return (_.filter($scope.filter.repositories, ["content_type", "deb"]));
        };
    }]
);
