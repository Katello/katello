/**
* Copyright 2014 Red Hat, Inc.
*
* This software is licensed to you under the GNU General Public
* License as published by the Free Software Foundation; either version
* 2 of the License (GPLv2) or (at your option) any later version.
* There is NO WARRANTY for this software, express or implied,
* including the implied warranties of MERCHANTABILITY,
* NON-INFRINGEMENT, or FITNESS FOR A PARTICULAR PURPOSE. You should
* have received a copy of GPLv2 along with this software; if not, see
* http://www.gnu.org/licenses/old-licenses/gpl-2.0.txt.
**/

describe('Factory: ContentHostTask', function() {
    var $httpBackend,
        task,
        pendingTask,
        ContentHostTask;

    beforeEach(module('Bastion.content-hosts'));
    beforeEach(module(function() {
        task = {id: 'TASK_ID', pending: false};
        pendingTask = {id: 'PENDING_TASK_ID', pending: true, system: {uuid: 'abcde'}};
    }));

    beforeEach(inject(function($injector) {
        $httpBackend = $injector.get('$httpBackend');
        ContentHostTask = $injector.get('ContentHostTask');
    }));

    afterEach(function() {
        $httpBackend.flush();
    });

    it('provides a way to get a content host task', function() {
        $httpBackend.expectGET('/katello/api/v2/systems/abcde/tasks/TASK_ID?paged=false').respond(task);
        ContentHostTask.get({ id: 'TASK_ID', contentHostId: 'abcde'}, function(results) {
            expect(results.id).toBe(task.id);
        });
    });

    it('provides a way to poll a task', function() {
        $httpBackend.expectGET('/katello/api/v2/systems/abcde/tasks/PENDING_TASK_ID?paged=false').respond(pendingTask);
        ContentHostTask.poll(pendingTask, function(results) {});
    });
});
