...
module Katello
  class Api::V2::AlternateContentSourcesControllerTest < ActionController::TestCase
    ...
    def test_create
      ...
      unique_name = "#{@acs.name}-#{SecureRandom.hex(4)}"
      expected = @acs.dup
      expected.name = unique_name
      expected.label = unique_name

      assert_sync_task(::Actions::Katello::AlternateContentSource::Create) do |acs, smart_proxies|
        assert_equal acs.attributes.except('id', 'label'), expected.attributes.except('id', 'label')
    ...
    def test_create_deb
      ...
      unique_name = "#{@deb_acs.name}-#{SecureRandom.hex(4)}"
      expected = @deb_acs.dup
      expected.name = unique_name
      expected.label = unique_name
      ...
      post :create, params: {
        ...
        deb_releases: @deb_acs.deb_releases,
        deb_components: @deb_acs.deb_components,
        deb_architectures: @deb_acs.deb_architectures,
        alternate_content_source_type: @deb_acs.alternate_content_source_type,
        verify_ssl: @deb_acs.verify_ssl,
        upstream_username: @deb_acs.upstream_username,
        upstream_password: @deb_acs.upstream_password,
      }
      assert_response :success
      assert_template 'api/v2/common/create'
    end
    ...
    def test_create_deb_simplified
      ...
      unique_name = "#{@deb_simplified.name}-#{SecureRandom.hex(4)}"
      expected = @deb_simplified.dup
      expected.name = unique_name
      expected.label = unique_name
    ...
    def test_create_rhui
      ...
      unique_name = "#{@acs_rhui.name}-#{SecureRandom.hex(4)}"
      expected = @acs_rhui.dup
      expected.name = unique_name
      expected.label = unique_name
    ...
    def test_create_simplified
      ...
      unique_name = "#{@acs_simplified.name}-#{SecureRandom.hex(4)}"
      expected = @acs_simplified.dup
      expected.name = unique_name
      expected.label = unique_name
