...
module Katello
  class Api::V2::FiltersControllerTest < ActionController::TestCase
    ...
    def test_create
      parameters = { :units => [{ :name => "testpkg", :version => "10.0" }] }
      ...
      assert_equal @content_view.filters.first.parameters[:units].first[:name], "testpkg"
      assert_equal @content_view.filters.first.parameters[:units].first[:version], "10.0"
    end
    ...
    def test_update_name
      put :update, :id => @filter, :name => "New Filter Name"
    ...
    def test_update_repositories
      ...
      put :update, :id => @filter, :repository_ids => [repository.id]
    ...
    def test_update_parameters
      parameters = { :units => [{ :name => "testpkg", :version => "10.0" }] }
      assert_empty @filter.parameters

      put :update, :id => @filter, :parameters => parameters

      assert_response :success
      assert_equal @filter.reload.parameters[:units].first[:name], "testpkg"
      assert_equal @filter.parameters[:units].first[:version], "10.0"
    end
    ...
    def test_update_protected
      ...
      assert_protected_action(:update, allowed_perms, denied_perms) do
        put :update, :id => @filter.id, :name => "new name"
      end
    end
