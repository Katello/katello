...
module Katello
  ...
  class BoundReposTest < ContentFacetBase
    ...
    let(:repo) { katello_repositories(:fedora_17_x86_64) }
    let(:short_repo) { katello_repositories(:generic_file) }
  ...
  class PopulateBootcFieldsFromFactsTest < ContentFacetBase
    ...
    def test_clear_bootc_fields_when_facts_explicitly_nil
      # First, populate bootc fields
      @content_facet.update!(
        bootc_booted_image: 'registry.example.com/bootc:original',
        bootc_booted_digest: 'sha256:0138b67750edb6ceaad90692b9ad2d25d44d814a0b6a6068d8017751a83764a0',
      ...
      # Upload facts with bootc keys present but explicitly nil values
      facts = {
        'bootc.booted.image' => nil,
        'bootc.booted.digest' => nil,
        'bootc.staged.image' => nil,
        'bootc.staged.digest' => nil,
      }

      parser = stub(facts: facts)
      Katello::Host::ContentFacet.populate_fields_from_facts(@host, parser, nil, nil)

      # Bootc fields should be cleared (set to nil)
      @content_facet.reload
      assert_nil @content_facet.bootc_booted_image
      assert_nil @content_facet.bootc_booted_digest
      assert_nil @content_facet.bootc_staged_image
      assert_nil @content_facet.bootc_staged_digest
    end
  end
end
