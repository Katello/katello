...
module Katello
  class RepositoryCreateTest < RepositoryTestBase
    ...
    def test_short_full_path_disabled
      ...
      assert_nil katello_repositories(:debian_9_amd64).short_full_path
    ensure
    ...
    def test_short_relative_path_enabled_for_deb_type
      original = Setting[:katello_pulp_short_paths]
      Setting[:katello_pulp_short_paths] = true

      assert_includes katello_repositories(:debian_9_amd64).short_relative_path, '/short/'
    ensure
      Setting[:katello_pulp_short_paths] = original
    end

    def test_short_relative_path_disabled_for_file_type
      original = Setting[:katello_pulp_short_paths]
      Setting[:katello_pulp_short_paths] = true

      assert_nil katello_repositories(:generic_file).short_relative_path
    ensure
      Setting[:katello_pulp_short_paths] = original
    end
