...
module Katello
  ...
  class SmartProxyPulp3Test < ActiveSupport::TestCase
    ...
    pulpcore_features.each_pair do |feature_name, repo_type|
      test "Pulpcore feature #{feature_name} is supported by smart proxy pulp primary" do
        SETTINGS[:katello][:use_pulp_2_for_content_type] = {}
        ...
        assert @primary.pulp3_repository_type_support?(repo_type.to_s),
          "Repostitory type \"#{repo_type}\" is not supported by smart proxy with capabilties names \"#{feature_name}\""
      end
    end
  end
end
