...
class RequiredCLIOptionsTests(CLIOptionTestCase):
    ...
    def test_missing_org_generates_error(self):
        self.assertRaises(Exception, self.action.process_options, ['remove_package', '--name=filter1', '--package_id=package1'])

    def test_missing_name_generates_error(self):
        self.assertRaises(Exception, self.action.process_options, ['remove_package', '--org=ACME', '--package_id=package1'])
    ...
    def test_no_error_if_org_and_name_provided(self):
        self.action.process_options(['remove_package', '--org=ACME', '--name=filter1', '--package_id=package1'])
