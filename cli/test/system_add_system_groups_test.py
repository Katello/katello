...
class RequiredCLIOptionsTests(CLIOptionTestCase):
    ...
    def test_missing_org_generates_error(self):
        self.assertRaises(Exception, self.action.process_options, ['add_system_groups', '--name=system_1', '--system_groups=Bob,SystemGroup1'])

    def test_missing_name_generates_error(self):
        self.assertRaises(Exception, self.action.process_options, ['add_system_groups', '--org=ACME', '--system_groups=SysG1'])

    def test_missing_system_groups_generates_error(self):
        self.assertRaises(Exception, self.action.process_options, ['add_system_groups', '--org=ACME', '--name=system_1'])

    def test_no_error_if_org_and_name_and_system_groups_provided(self):
        self.action.process_options(['add_system_groups', '--org=ACME', '--name=system_1', '--system_groups=Here,There,Where'])
