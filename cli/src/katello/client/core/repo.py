...
class Sync(SingleRepoAction):
    ...
    def run(self):
        ...
        if task.succeeded():
            ...
            print _("Repo [ %(repo_name)s ] failed to sync: %(sync_errors)s" % {'repo_name':repo['name'], 'sync_errors':format_sync_errors(task)} )
...
class AddRemoveFilter(SingleRepoAction):  # pylint: disable=R0904
    ...
    def update_filters(self, repo, filters, filter_name):
        if self.addition:
            ...
            message = _("Added filter [ %(filter_name)s ] to repository [ %(repo_name)s ]" % {'filter_name':filter_name, 'repo_name':repo["name"]})
        else:
            filters.remove(filter_name)
            message = _("Removed filter [ %(filter_name)s ] to repository [ %(repo_name)s ]" % {'filter_name':filter_name, 'repo_name':repo["name"]})
