...
class BaseAction(Action):
    ...
    def main(self, args, command_name=None, parent_usage=None):
        ...
        try:
            ...
            print ""
            print _("You have: [%(expectedHost)s] configured but got: [%(actualHost)s] from the server.") % {'expectedHost':wh.expectedHost, 'actualHost':wh.actualHost}
            print ""
...
def check_url(option, opt, value):
    ...
    if not url_parsed.scheme in schemes:                                 # pylint: disable=E1101
        ...
        raise OptionValueError(_('option %(opt)s: has to start with %(formatted_schemes)s') % {'opt':opt, 'formatted_schemes':formatted_schemes})
