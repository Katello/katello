    ...
    def run(self):
        ...
        test_record(new_system_group,
            _("Successfully copied system group [ %(source_system_group_name)s ] to [ %(new_system_group_name)s ]") %
                       {'source_system_group_name':source_system_group['name'], 'new_system_group_name':new_system_group['name']},
...
class HistoryTasks(SystemGroupAction):
    ...
    def run(self):
        ...
        if history == None:
            print >> sys.stderr, _("Could not find job [ %(job_id)s ] for system group [ %(system_group_name)s ]") % {'job_id':job_id, 'system_group_name':system_group_name}
            return os.EX_DATAERR
