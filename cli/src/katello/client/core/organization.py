...
class Delete(OrganizationAction):
    ...
    def run(self):
        ...
        if task.succeeded():
            ...
            print _("Organization [ %(name)s ] deletion failed: %(format_task_errors)s" % {'name':name, 'format_task_errors':format_task_errors(task.errors())} )
...
class AddDefaultSystemInfo(OrganizationAction):
    ...
    def run(self):
        ...
        if response:
            print _("Successfully added default custom info key [ %(keyname)s ] to Org [ %(org_name)s ]") % {'keyname':keyname, 'org_name':org_name}
        else:
            print _("Could not add default custom info key [ %(keyname)s ] to Org [ %(org_name)s ]") % {'keyname':keyname, 'org_name':org_name}
...
class RemoveDefaultSystemInfo(OrganizationAction):
    ...
    def run(self):
        ...
        if not keyname in response:
            print _("Successfully removed default custom info key [ %(keyname)s ] for Org [ %(org_name)s ]") % {'keyname':keyname, 'org_name':org_name}
        else:
            print _("Could not remove default custom info key [ %(keyname)s ] for Org [ %(org_name)s ]") % {'keyname':keyname, 'org_name':org_name}
...
class ApplyDefaultSystemInfo(OrganizationAction):
    ...
    def run(self):
        ...
        if response:
            print _("Successfully applied default custom info keys to [ %(sys_count)d ] systems in Org [ %(org_name)s ]")\
