...
class List(ProductAction):
    ...
    def run(self):
        ...
        if prov_name:
            ...
            self.printer.set_header(_("Product List For Organization %(org_name)s, Environment '%(env_name)s'") % {'org_name':org_name, 'env_name':env["name"]})
    ...
    def run(self):
        ...
        if task.failed():
            errors = [t["result"]['errors'][0] for t in task.get_hashes() if t['state'] == 'error' and
                                                                             ...
                                                                             "errors" in t["result"]]
            print _("Product [ %(prod_name)s ] failed to sync: %(errors)s" % {'prod_name';prod["name"], 'errors':errors})
...
class Promote(SingleProductAction):
    ...
    def run(self):
        ...
        try:
            ...
            if task.succeeded():
                print _("Product [ %(prod_name)s ] promoted to environment [ %(envName)s ] " % {'prod_name':prod["name"], 'envName':envName})
                returnCode = os.EX_OK
            else:
                print _("Product [ %(prod_name)s ] promotion failed: %(task_errors)s" % {'prod_name':prod["name"], 'task_errors':format_task_errors(task.errors())} )
...
class AddRemoveFilter(SingleProductAction):
    ...
    def update_filters(self, org_name, product, filters, filter_name):
        if self.addition:
            ...
            message = _("Added filter [ %(filter_name)s ] to product [ %(product)s ]") % {'filter_name':filter_name, 'product':product["name"]}
        else:
            filters.remove(filter_name)
            message = _("Removed filter [ %(filter_name)s ] from product [ %(product)s ]") % {'filter_name':filter_name, 'product':product["name"]}
