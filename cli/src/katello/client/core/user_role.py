...
class Info(UserRoleAction):
    ...
    def formatPermission(cls, p, details=True):
        if details:
            ...
            return _("(param_name)%s\n\tfor: %(type_in)s\n\tverbs: %(verbs)s\n\ton: %(tags)s") % {'param_name':p['name'], 'type_in':type_in, 'verbs':verbs, 'tags':tags}
        else:
            return p['name']
...
class AddLdapGroup(UserRoleAction):
    ...
    def run(self):
        ...
        print _("Successfully added LDAP group [ %(group_name)s ] to the user role [ %(name)s ]") % {'group_name':group_name, 'name':name}
        return os.EX_OK
...
class RemoveLdapGroup(UserRoleAction):
    ...
    def run(self):
        ...
        print _("Successfully removed LDAP group [ %(group_name)s ] from the user role [ %(name)s ]") % {'group_name':group_name, 'name':name}
        return os.EX_OK
