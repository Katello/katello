...
class Info(PackageGroupAction):
    ...
    def run(self):
        ...
        if group == None:
            system_exit(os.EX_DATAERR, _("Package group [%(groupid)s] not found in repo [%(repoid)s]") % {'groupid':groupid, 'repoid':repoid})
...
class CategoryInfo(PackageGroupAction):
    ...
    def run(self):
        ...
        if category == None:
            system_exit(os.EX_DATAERR, _("Package group category [%(categoryId)s] not found in repo [%(repoid)s]") % {'categoryId':categoryId, 'repoid':repoid})
