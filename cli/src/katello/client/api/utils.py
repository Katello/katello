...
def get_system_group(org_name, system_group_name):
    ...
    if system_group == None:
        raise ApiDataError(_("Could not find system group [ %(system_group_name)s ] within organization [ %(org_name)s ]") % \
...
def get_system(org_name, sys_name, env_name=None, sys_uuid=None):
    ...
    if sys_uuid:
        ...
        if systems is None:
            raise ApiDataError(_("Could not find System [ %(sys_name)s ] in Org [ %(org_name)s ]") % {'sys_name':sys_name, 'org_name':org_name})
        elif len(systems) != 1:
            raise ApiDataError(_("Found ambiguous Systems [ %(sys_uuid)s ] in Org [ %(org_name)s ]") % {'sys_uuid':sys_uuid, 'org_name':org_name})
    elif env_name is None:
        systems = system_api.systems_by_org(org_name, {'name': sys_name})
        if systems is None or len(systems) == 0:
            raise ApiDataError(_("Could not find System [ %(sys_name)s ] in Org [ %(org_name)s ]") % {'sys_name':sys_name, 'org_name':org_name})
        elif len(systems) != 1:
            raise ApiDataError( _("Found ambiguous Systems [ %(sys_name)s ] in Environment [ %(env_name)s ] in Org [ %(org_name)s ], "\
        ...
        if systems is None:
            raise ApiDataError(_("Could not find System [ %(sys_name)s ] in Environment [ %(env_name)s ] in Org [ %(org_name)s ]") %
