...
describe Api::ActivationKeysController do
  ...
  describe "add system groups to an activation key" do
    ...
    let(:req) { post :add_system_groups, :id => @activation_key.id, :organization_id => @organization.cp_key }
    let(:authorized_user) { user_with_manage_permissions }
    let(:unauthorized_user) { user_without_manage_permissions }
    it_should_behave_like "protected action"
    ...
    it "should update the system groups attached to the activation key" do
      ...
      post :add_system_groups, :id => @activation_key.id, :organization_id => @organization.cp_key, :activation_key => { :system_group_ids => ids }
  ...
  describe "remove system groups from an activation key" do
    ...
    let(:req) { delete :remove_system_groups, :id => @activation_key.id, :organization_id => @organization.cp_key }
    let(:authorized_user) { user_with_manage_permissions }
    let(:unauthorized_user) { user_without_manage_permissions }
    it_should_behave_like "protected action"
    ...
    it "should update the system groups the system is in" do
      ids = [@system_group_1.id, @system_group_2.id]
      delete :remove_system_groups, :id => @activation_key.id, :organization_id => @organization.cp_key, :system => { :system_group_ids => ids }
      @activation_key.system_group_ids.should be_empty
    end

    it "should throw a 404 is passed in a bad system group id" do
      ids = [90210]
      delete :remove_system_groups, :id => @activation_key.id, :organization_id => @organization.cp_key, :activation_key => { :system_group_ids => ids }
      response.status.should == 404
    end
  end
end
