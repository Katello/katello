...
class ContentViewDefinition < ContentViewDefinitionBase
  ...
  def archive
    ...
    cvd_archive.repositories            = self.repositories.map(&:clone)
    cvd_archive.products                = self.products.map(&:clone)
    cvd_archive.component_content_views = self.component_content_views.map(&:clone)
