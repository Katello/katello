...
module Glue::Candlepin::ContentViewEnvironment
  ...
  module InstanceMethods
    ...
    def set_environment
      Rails.logger.info _("Creating content view environment in candlepin: %s") % self.label
      ...
      Rails.logger.error _("Failed to create candlepin content view environment %s") %
                           "#{self.label}: #{e}, #{e.backtrace.join("\n")}"
      raise e
    end

    def del_environment
      Rails.logger.info _("Deleting content view environment in candlepin: %s") % self.label
      Resources::Candlepin::Environment.destroy(self.cp_id)
    rescue => e
      Rails.logger.error _("Failed to delete candlepin content view environment %s") %
                           "#{self.label}: #{e}, #{e.backtrace.join("\n")}"
      raise e
    end
