...
class ContentSearch::ContentViewComparison < ContentSearch::Search
  ...
  def product_rows(products, cols = [])
    products.inject([]) do |product_rows, product|
      ...
      cols.each do |key, col|
        ...
        row.cols[key] = ContentSearch::Column.new(:display => total, :id => key) if total && total > 0
      end
  ...
  def repo_rows(library_repos, cols = []) repo_rows = meta_rows = []
    repo_rows = []
    ...
    library_repos.each do |library_repo|
      ...
      if packages.length > page_size
        meta_row =  ContentSearch::MetadataRow.new(:total => packages.length,
