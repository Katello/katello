...
module Authorization::Environment
  ...
  module ClassMethods
    ...
    def any_viewable_for_promotions?(org)
      ...
      ::User.allowed_to?(CHANGE_SETS_READABLE + CONTENTS_READABLE, :environments, org.lifecycle_environment_ids, org, true)
    end

    def any_contents_readable?(org, skip_library = false)
      ids = org.lifecycle_environment_ids
    ...
    def authorized_items(org, verbs, resource = :environments)
      ...
      if ::User.allowed_all_tags?(verbs, resource, org)
        ...
        where("#{Katello::LifecycleEnvironment.table_name}.id in (#{User.allowed_tags_sql(verbs, resource, org)})")
      end
    end
