  ...
  module Authorization::Repository
    ...
    module ClassMethods
      def readable
        ...
        in_content_views = Repository.joins(:content_view_repositories).where("#{ContentViewRepository.table_name}.content_view_id" => ContentView.readable).pluck(:id)
        in_versions = Repository.joins(:content_view_version).where("#{Katello::ContentViewVersion.table_name}.content_view_id" => ContentView.readable).pluck(:id)
