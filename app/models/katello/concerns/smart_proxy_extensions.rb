  ...
  module Concerns
    ...
    module SmartProxyExtensions
      ...
      def repos_content_count(repos)
        new_content_counts = (content_counts.deep_dup || { content_view_versions: {} }).with_indifferent_access
        repos.each do |repo|
          ...
          new_content_counts[:content_view_versions][repo.content_view_version_id.to_s] ||= { repositories: {}}
          new_content_counts[:content_view_versions][repo.content_view_version_id.to_s][:repositories][repo.id.to_s] = translated_counts
        end
        version_ids_available_to_proxy = lifecycle_environments&.first&.content_view_versions&.pluck(:id)&.map(&:to_i)
        version_ids_in_count_map = new_content_counts[:content_view_versions].keys&.map(&:to_i)
        version_ids_to_remove = version_ids_in_count_map - version_ids_available_to_proxy
        version_ids_to_remove.each { |id| new_content_counts[:content_view_versions].delete(id.to_s) }
