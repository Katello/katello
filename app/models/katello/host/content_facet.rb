    ...
    class ContentFacet < Katello::Model
      ...
      def assign_single_environment(
        ...
        if content_view_environment.nil?
          ...
          fail _("Unable to find a content view with ID %s") % content_view_id if cv_name.nil?
          env_name = ::Katello::KTEnvironment.find_by(:id => lifecycle_environment_id)&.name
          fail _("Unable to find a lifecycle environment with ID %s") % lifecycle_environment_id if env_name.nil?
          hypothetical_cve_label = "%s/%s" % [env_name, cv_name]
          fail _("Cannot assign content view environment %s: The content view has either not been published or has not been promoted to that lifecycle environment.") % hypothetical_cve_label
        end
