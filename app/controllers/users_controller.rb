...
class UsersController < ApplicationController
  ...
  def edit 
    render :partial=>"edit", :layout => "tupane_layout", :locals=>{:user=>@user, :editable=>@user.editable?, :name=>controller_name}
  end
  ...
  def create
    begin
      ...
      render :partial=>"common/list_item", :locals=>{:item=>@user, :accessor=>"id", :columns=>["username"], :name=>controller_name}
    rescue Exception => error
      errors error
  ...
  def destroy
    ...
    begin
      ...
      if @user.destroyed?
        ...
        render :partial => "common/list_remove", :locals => {:id => @id, :name=>controller_name} and return
      end
      errors "", {:list_items => @user.errors.to_a}
      render :text => @user.errors, :status=>:ok
    rescue Exception => error
