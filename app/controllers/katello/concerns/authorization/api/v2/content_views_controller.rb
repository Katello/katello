  ...
  module Concerns::Authorization::Api::V2::ContentViewsController
    ...
    def authorize_remove
      ...
      authorize_remove_versions(view, options) &&
        ...
        authorize_hostgroup_content_view(view, options) &&
        authorize_hostgroup_environments(view, options)
    end
    ...
    def authorize_hostgroup_content_view(view, options)
      hostgroup_content_view_id = options[:hostgroup_content_view_id]
      if hostgroup_content_view_id
        hg_content_view = ContentView.where(:organization_id => view.organization,
                                            :id => hostgroup_content_view_id).first
        fail HttpErrors::NotFound, _("Couldn't find host group content view id '%s'") % hostgroup_content_view_id unless hg_content_view
        # deny if we cannot reassign hostgroups to the new content view id
        return deny_access unless hg_content_view.readable?
      end
      true
    end

    def authorize_hostgroup_environments(view, options)
      hostgroup_environment_id = options[:hostgroup_environment_id]
      if hostgroup_environment_id
        hg_env = KTEnvironment.where(:organization_id => view.organization,
                                     :id => hostgroup_environment_id).first
        fail HttpErrors::NotFound, _("Couldn't find host group environment '%s'") % hostgroup_environment_id unless hg_env
        # deny if we cannot reassign hostgroups to the new env id
        return deny_access unless hg_env.readable?
      end
      true
    end
    ...
    def authorize_hostgroup_reassignment(view, env_ids, options)
      ...
      unless options[:hostgroup_content_view_id] && options[:hostgroup_environment_id]
        fail _("Unable to reassign host groups. Please provide hostgroup_content_view_id and hostgroup_environment_id.")
      end
