  ...
  class Api::Registry::RegistryProxiesController < Api::V2::ApiController
    ...
    def authenticate_cert_request
      if cert_present?
        ...
        if auth
          ...
          @client = Katello::Host::ContentFacet.find_by(uuid: uuid)&.host
        end
      end
    end
    ...
    def set_user_by_token(token, redirect_on_failure = true)
      if token
        ...
        return true if @client && User.current
      end
      false
    end
    ...
    def find_readable_repository
      ...
      if require_user_authorization?(repository)
        repository = Repository.readable_docker_catalog(@client).find_by(container_repository_name: params[:repository])
      end
      repository
    end
    ...
    def v1_search
      ...
      search_results = scoped_search(Repository.readable_docker_catalog(@client).distinct,
    ...
    def catalog
      repositories = Repository.readable_docker_catalog(@client).collect do |repository|
