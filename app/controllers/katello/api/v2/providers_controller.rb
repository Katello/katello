...
class Api::V2::ProvidersController < Api::V2::ApiController
  ...
  def_param_group :provider_group do
    param :name, String, :required => true, :desc => "name of the provider"
  ...
  param :provider_type, ["Red Hat", "Custom"], :desc => "filter the index by provider type"
  def index
    options = sort_params
    ...
    respond_for_index(:collection => item_search(Provider, params, options))
  end
  
  api :GET, "/providers/:id", "Show a provider"
  param :id, :identifier, :desc => "Id of provider", :required => true
  def show
    respond
  end
  ...
  def destroy
    ...
    if @provider.destroyed?
      respond_for_destroy :message => _("Deleted provider [ %s ]") % @provider.name
    else
