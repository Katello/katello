module Katello
  class Api::V2::HostCollectionsController <  Api::V2::ApiController
    ...
    before_filter :load_search_service, :only => [:index, :hosts]
    ...
    param :id, :identifier, :desc => N_("Id of the host collection"), :required => true
    param :host_ids, Array, :required => false, :desc => N_("List of host ids to be in the host collection")
    ...
    api :GET, "/host_collections/:id/hosts", N_("List hosts in the host collection")
    param :id, :identifier, :desc => N_("Id of the host collection"), :required => true
    def hosts
      hosts = ::Host.authorized(:edit_host).where("#{::Host.table_name}.id" => @host_collection.host_ids)
      collection = scoped_search(hosts, :name, :desc, :resource_class => ::Host)
      respond_for_index(:collection => collection, :template => '../hosts/index')
    end

    api :PUT, "/host_collections/:id/add_hosts", N_("Add host to the host collection"), :deprecated => true
    param :id, :identifier, :desc => N_("Id of the host collection"), :required => true
    ...
    api :PUT, "/host_collections/:id/remove_hosts", N_("Remove hosts from the host collection"), :deprecated => true
    param :id, :identifier, :desc => N_("Id of the host collection"), :required => true
    param :host_ids, Array, :desc => N_("Array of host ids")
    def remove_hosts
      @hosts = ::Host::Managed.authorized(:edit_host).where(:id => params[:host_ids])
    ...
    def find_host
      @host = ::Host::Managed.find(params[:host_id]) if params[:host_id]

      # This is necessary until the content host UI is updated to use host id instead of system UUID.
      if params[:system_id]
