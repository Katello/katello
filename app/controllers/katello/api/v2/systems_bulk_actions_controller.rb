...
class Api::V2::SystemsBulkActionsController < Api::V2::ApiController
  ...
  before_filter :validate_group_permissions, :only => [:bulk_add_system_groups, :bulk_remove_system_groups]
  ...
  def rules
    ...
    hash[:bulk_add_system_groups] = lambda{true}
    hash[:bulk_remove_system_groups] = lambda{true}
  ...
  def readable_filters
    {:or => [
        {:terms => {:environment_id => KTEnvironment.systems_editable(@organization).collect { |item| item.id } }},
        {:terms => {:system_group_id => SystemGroup.systems_editable(@organization).collect { |item| item.id } }},
      ]
    }
  end

  def validate_group_permissions
    invalid_perms = @system_groups.select{ |group| !group.editable? }.collect{ |group| group.name }
