...
class Api::UeberCertController < Api::ApiController
  ...
  def create
    ...
    raise HttpErrors::Conflict _("Ueber certificate has already been generated.") unless existing_ueber_product.nil?
    #return regenerate_ueber_certificate(existing_ueber_product[:id]) unless existing_ueber_product.nil?
    render :json => generate_ueber_certificate
  end

  def show
    render :json => Candlepin::Consumer.entitlements(@ueber_consumer.uuid).to_json
  end
  ...
  def generate_ueber_certificate
    ...
    ueber_content = Candlepin::Content.create({
      :name => "ueber_content",
      :contentUrl => "/#{@organizaton.name}",
      :gpgUrl => "",
      :type => "yum",
      :label => "#{ueber_product[:id]}_#{name}",
      :vendor => "Custom"
    })
    ...
    Candlepin::Consumer.consume_entitlement(ueber_consumer.uuid, ueber_pool[:id], 1)
  end
