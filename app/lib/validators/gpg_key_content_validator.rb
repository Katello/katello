  ...
  class GpgKeyContentValidator < ActiveModel::EachValidator
    def validate_each(record,attribute,value)
      if value
        ...
        unless gpg_key_line_array.first.match(/-{5}BEGIN PGP PUBLIC KEY BLOCK-{5}/) && gpg_key_line_array.last.match(/-{5}END PGP PUBLIC KEY BLOCK-{5}/)
    ...
    def self.validate_line_length(record,attribute,value)
      value.each_line do |line|
        record.errors[attribute] << _("must contain valid  Public GPG Key") if line.length > GpgKey::MAX_LINE_LENGTH
      end
    end
