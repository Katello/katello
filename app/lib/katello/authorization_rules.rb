...
module AuthorizationRules
  ...
  def authorize(ctrl = params[:controller], action = self.action_name)
    ...
    raise Errors::SecurityViolation, "User #{current_user.login} is not allowed to access #{params[:controller]}/#{params[:action]}"
  end

  def rules
    raise Errors::SecurityViolation, "Rules not defined for  #{current_user.login} for #{params[:controller]}/#{params[:action]}"
  end
  ...
  def check_array_params(rule, params)
    (params.keys - %w(_method controller action commit authenticity_token utf8 search format) - rule.collect { |r| r.to_s })
  end
