  ...
  module Concerns
    ...
    module BaseTemplateScopeExtensions
      ...
      def load_errata_applications_from_db(filter_errata_type: nil, include_last_reboot: 'yes', since: nil, up_to: nil, status: 'all', host_filter: nil)
        ...
        host_ids = applications.pluck(:host_id).uniq
        ...
        # Process each application record (which may contain multiple errata)
        applications.find_each(batch_size: 1000) do |app|
          # Get all errata for this application
          errata = Katello::Erratum.where(id: app.errata_ids)

          # Filter by errata type if specified
          if filter_errata_type.present? && filter_errata_type != 'all'
            errata = errata.where(errata_type: filter_errata_type)
          end
