  ...
  module Katello
    module ContentView
      class Remove < Actions::EntryAction
        ...
        # hostgroup_content_view_id - content view to reassociate host groups with
        # hostgroup_environment_id - environment to reassociate host groups with
        ...
        def validate_options(_content_view, cv_envs, versions, options)
          ...
          if hostgroups_exist && !cve_exists?(options[:hostgroup_environment_id], options[:hostgroup_content_view_id])
            fail _("Unable to reassign host groups. Please check hostgroup_content_view_id and hostgroup_environment_id.")
          end
        end
